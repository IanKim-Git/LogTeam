...
public class ProgUserDAO {
	...
	// ###단위 테스트###
	/*
	 * public static void main(String[] args) { ProgUserDAO p = new
	 * ProgUserDAO(); int result = 0; System.out.println("### 로그인 테스트 ###");
	 * System.out.println(p.userIdPwCheck("a@a.a", "1234"));
	 * 
	 * // System.out.println("### 회원가입 테스트 ###"); //new ProgUserBean(uemail,
	 * uname, upw) //new ProgUserBean(uemail, uname, upw, uphone) // insert into
	 * proguser(uemail, uname, upw, uphone) values(#{uemail} , #{uname} ,
	 * #{upw}, #{uphone}) // result = p.userSignIn(new ProgUserBean("d@d.d",
	 * "박다은", "1234", "123-456-7890")); // result = p.userSignIn(new
	 * ProgUserBean("e@e.e", "박소현", "1234", "")); // result = p.userSignIn(new
	 * ProgUserBean("f@f.f", "박상준", "1234", "000-0000-0000")); // result =
	 * p.userSignIn(new ProgUserBean("g@g.g", "박상준", "1234", "")); // if(result
	 * > 0){ // System.out.println("회원가입 성공"); // }else{ //
	 * System.out.println("회원가입 실패"); // } //
	 * System.out.println("### 모든 사용자 출력 테스트 ###"); // List<ProgUserBean> au =
	 * p.allUsers(); // for(ProgUserBean pu : au){ // System.out.println(pu); //
	 * }
	 * 
	 * // System.out.println("### 이메일 중복 테스트 ###"); //
	 * System.out.println(p.userIdCheck("zz")); //
	 * System.out.println("### 회원 정보 수정"); // System.out.println("***수정 전***");
	 * // System.out.println(p.userIdCheck("d@d.d")); // //
	 * System.out.println("***수정 후***"); // p.userUpdate(new
	 * ProgUserBean("d@d.d", "박다은", "1234", "123-456-7890"));
