...
public class ProgUserController {
	...
	@ResponseBody
	public String insert(@RequestParam("email2") String uemail,
			@RequestParam("name") String uname,
			@RequestParam("pw2") String upw,
			@RequestParam("phone") String uphone, Model model) {
		// System.out.println("Eee");
		String resultMsg = "signInFail";// 저장 실패시 응답되는 데이터
		System.out.println(uname);

		int result = puService.userSignIn(new ProgUserBean(uemail, uname, upw,
				uphone));
		if (result > 0) {
	...
	@ResponseBody
	public String updateUser(@RequestParam("email2") String uemail,
			@RequestParam("name") String uname,
			@RequestParam("pw2") String upw,
			@RequestParam("phone") String uphone, Model model) {
		String resultMsg = "signInFail";// 저장 실패시 응답되는 데이터
